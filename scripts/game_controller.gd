...
var next_ball: Ball ## Next ball in line
@export var next_ball_pos: Vector2 = Vector2(880, 200) ## Center pos of next ball preview
...
## Generates a new [Ball] and saves it to the [member GameController.next_ball]
## [br][br]
## [Ball] is [member GameController.next_ball_pos] [br]
## Level is generated by [method GameController.GetNextLevel]
func ReadyNextBall():	
	var level = GetNextLevel()
	var new_ball = CreateBall(next_ball_pos, level)
	next_ball = new_ball
	self.add_child.call_deferred(next_ball)

func SwitchToNextBall():
	if (next_ball == null):
		ReadyNextBall()
	ball = next_ball
	ball.global_position = Vector2(GetMouseXPos(), ball_top_offset)
